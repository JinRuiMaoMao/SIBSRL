...
(function () {
  ...
  function buildBootSegments(start, target) {
    ...
    var stutterEnd = Math.min(target, STUTTER_MAX);
    if (stutterEnd > stutterStart) segments.push({ from: stutterStart, to: stutterEnd, stutter: true });
    if (target > STUTTER_MAX) {
      var tailStart = Math.max(start, STUTTER_MAX);
  ...
  async function smoothProgressTo(start, target, label) {
    ...
    while (current < target - 0.5) {
      ...
      var step = Math.max(1.2, Math.min(remaining, remaining * (0.14 + Math.random() * 0.18)));
  ...
  async function stutterSegmentTo(start, target, label) {
    var current = start;
    var floor = Math.max(STUTTER_MIN - 4, start - 3);
    setProgress(current, label, 'hold');
