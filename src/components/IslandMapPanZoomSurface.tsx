...
import {
  ...
  shouldCapIslandMapRasterDecode,
} from '../utils/islandMapRasterLoad'
...
export function IslandMapPanZoomSurface({
  ...
  useEffect(() => {
    if (src === displayedSrcRef.current && displayImageSrc) {
    ...
    const waitForSafariImageRelease = () =>
    ...
    const run = async () => {
      ...
      const logicalSize = getIslandMapLogicalPixelSize(src)
      if (!logicalSize) {
        setMapLayerLoading(false)
        onMapLayerLoadingChangeRef.current?.(false)
        return
  ...
  }, [commitMapLayer, displayImageSrc, releaseDomMapImage, src])
  ...
  const onPointerDown = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (drawMode) {
      ...
      onDrawMapClick?.(
        viewportClientToNormalized(event.clientX, event.clientY, rect, livePanZoom, imageSize),
      )
      return
    }
  ...
  useEffect(() => {
    if (!draggingNodeId || !panZoom || !imageSize) return
    ...
    const onPointerMove = (event: PointerEvent) => {
      ...
      const livePanZoom = panZoomRef.current ?? panZoom
      const point = viewportClientToNormalized(event.clientX, event.clientY, rect, livePanZoom, imageSize)
      onPathNodeDrag?.(drag.nodeId, point)
    }
    ...
    const finishDrag = (event: PointerEvent) => {
      ...
      const point = viewportClientToNormalized(event.clientX, event.clientY, rect, livePanZoom, imageSize)
      if (drag.moved) {
        onPathNodeDragEnd?.(drag.nodeId, point)
      } else {
        onPathNodeClick?.(drag.nodeId)
      }
  ...
  const clientToNormalized = useCallback(
    (clientX: number, clientY: number) => {
      const viewport = viewportRef.current
      const livePanZoom = panZoomRef.current ?? panZoom
      if (!viewport || !imageSize || !livePanZoom) return [0.5, 0.5] as WorldMapPoint
      const rect = viewport.getBoundingClientRect()
      return viewportClientToNormalized(clientX, clientY, rect, livePanZoom, imageSize)
    },
    [imageSize, panZoom],
  )
  ...
  const onImageLoad = (event: React.SyntheticEvent<HTMLImageElement>) => {
    ...
    const cachedSize = imageSizeCacheRef.current.get(layerUrl)
    const capActive = shouldCapIslandMapRasterDecode()
    if (!capActive && !islandMapImageMatchesLayerUrl(image, layerUrl)) return

    const size = cachedSize ?? readImageSize(image)
    if (!size) return
    if (imageSize?.width === size.width && imageSize?.height === size.height) return
  ...
  const mapImageSrc = displayImageSrc || displayedSrc
